  ...
  <div class="chatbot-wrapper">
    ...
    <transition name="panel-slide">
      <div v-if="isOpen" class="chat-panel">
        ...
        <p class="chat-footer">Powered by Barangay Smart Connect</p>
      </div>
    </transition>
  </div>
</template>
...
const sendMessage = async () => {
  ...
  try {
    ...
    messages.value.push({
      role: 'assistant',
      content: 'Sorry, I am temporarily unavailable. Please try again or visit the barangay hall directly.',
      time: formatTime(),
    })
  } finally {
...
.chat-toggle:hover {
  ...
  box-shadow: 0 8px 30px rgba(13, 92, 143, 0.55);
}
...
@keyframes bounceOut { from { transform: scale(1); opacity: 1; } to { transform: scale(0.5); opacity: 0; } }
...
@keyframes panelOut { from { opacity: 1; transform: scale(1); } to { opacity: 0; transform: scale(0.9) translateY(10px); } }
...
@media (max-width: 480px) {
  ...
  /* Toggle — sits just above the bottom nav (64px) */
  .chatbot-wrapper {
    bottom: 80px;
    right: 16px;
  }
  ...
  .chat-toggle {
    width: 48px;
    height: 48px;
    box-shadow: 0 4px 16px rgba(13, 92, 143, 0.4);
  }
  .toggle-icon svg { width: 22px; height: 22px; }

  /* Panel — compact bottom sheet, not full screen */
  .chat-panel {
    position: fixed;
    bottom: 148px;   /* clears bottom nav (64px) + toggle (48px) + gap */
    right: 12px;
    left: 12px;
    width: auto;
    height: 62dvh;
  ...
  .avatar svg { width: 32px; height: 32px; }
  .header-info h3 { font-size: 13px; }
  .status-text { font-size: 11px; }
  .close-btn { width: 28px; height: 28px; border-radius: 7px; }
  .close-btn svg { width: 14px; height: 14px; }

  /* Tighter messages */
  .chat-messages { padding: 12px 12px; gap: 10px; }
  .welcome-icon { font-size: 26px; margin-bottom: 6px; }
  .welcome-block h4 { font-size: 13px; margin-bottom: 4px; }
  .welcome-block p { font-size: 12px; margin-bottom: 10px; }
  .chip { font-size: 11px; padding: 4px 10px; }
  .quick-chips { gap: 5px; }

  /* Smaller bubbles */
  .bubble { font-size: 12.5px; padding: 9px 11px; }
  ...
  textarea { font-size: 13px; padding: 8px 11px; border-radius: 10px; }
